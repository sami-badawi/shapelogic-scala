...
object EdgeTracerColorSpec {
  ...
  def getInstance(
    ...
    imageTry match {
      case Success(image) => {
        val edgeTracer = EdgeTracerColor(image, maxDistance, traceCloseToColor)
...
/**
 * Test of EdgeTracerColor.<br />
 *
 * @author Sami Badawi
 *
 */
class EdgeTracerColorSpec extends FunSuite with BeforeAndAfterEach {
  import EdgeTracerColorSpec._
  ...
  test("RedboxInverse") {
    ...
    val edgeTracer: IEdgeTracer = EdgeTracerColor.fromBufferImage(image, foregroundColorClose, 10, false)
  ...
  test("Blackbox") {
    ...
    assertResult(foregroundColor.toSeq) { image.getPixel(2, 2).toSeq } //Foreground
    val edgeTracer: IEdgeTracer = EdgeTracerColor.fromBufferImage(image, foregroundColor, 10, true)
  ...
  test("I") {
    val filename = "I"
    val imageTry: Try[BufferImage[Byte]] = imageLoad.loadBufferImageTry(imageLoad.filePath("./src/test/resources/data/images/smallThinLetters", filename, ".png"))
    if (imageTry.isFailure) {
      imageTry.failed.get.printStackTrace()
    }
    val image: BufferImage[Byte] = imageTry.get
